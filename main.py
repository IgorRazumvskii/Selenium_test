...
def myfunc(myarray, number):
    ...
    return newarray
z = myfunc(myarray, number)

def sel(*k):
    s = k
    print(*s)#просмотр элементов
    ...
    # вход на тест
    driver.get("https://lms.mtuci.ru/lms/mod/quiz/view.php?id=61613")
    time.sleep(1)
...
for i in range(8):
    tmp = 8
    th = Thread(target=sel, name=("th-{}".format(i)), args=(z[1+int((len(z)-1)/tmp*i):len(z)-1+int((len(z)-1)/tmp*i):],))
    th.start()